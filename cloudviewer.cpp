...
void CloudViewer::getPointCloudDisplayerPtrs(const QVector<int> &indices, QVector<PointCloudDisplayer::Ptr>& out)
{
    for(const auto& i:indices){
        if(i>=clouds.size() || i < 0){
            std::cerr<<CONSOLE_WRITE_RED<<"index out of range["<<Q_FUNC_INFO<<"]"<<CONSOLE_WRITE_RED_END<<std::endl;
            continue;
        }
        out.push_back(clouds[i]);
    }
}
