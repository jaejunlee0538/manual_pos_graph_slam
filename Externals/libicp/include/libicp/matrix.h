...
//typedef float  FLOAT;    // single precision
    ...
    public:
        ...
        static Matrix rotMat2D(const FLOAT &angle);
        ...
        const FLOAT &operator()(int i, int j) const { return val[i][j]; }

        FLOAT &operator()(int i, int j) { return val[i][j]; }
    ...
    // compute a quaternion from 'm' under an assumption for m being a rotation matrix.
    // return false, if m is not 3x3 or 4x4 matrix.
    bool toQuaternion(const Matrix& m, FLOAT &x, FLOAT &y, FLOAT &z, FLOAT &w);
}
#endif // MATRIX_H
