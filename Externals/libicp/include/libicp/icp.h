...
namespace libicp {
    ...
    public:
        ...
        /*
         * Issue : Icp instance generate SIGSEGV when 'M' contains redundant point data.
         */
        // constructor
        // input: M ....... pointer to first model point
        //        M_num ... number of model points
        //        dim   ... dimensionality of model points (2 or 3)
        Icp(const double *M, const int32_t M_num, const int32_t dim);
        ...
        int fit(const double *T, const int32_t T_num, Matrix &R, Matrix &t, const double indist);

    private:

        // iterative fitting
        void fitIterate(const double *T, const int32_t T_num, Matrix &R, Matrix &t, const std::vector<int32_t> &active);

        // inherited classes need to overwrite these functions
        virtual double fitStep(const double *T, const int32_t T_num, Matrix &R, Matrix &t,
                               const std::vector<int32_t> &active) = 0;

        virtual std::vector<int32_t> getInliers(const double *T, const int32_t T_num, const Matrix &R, const Matrix &t,
