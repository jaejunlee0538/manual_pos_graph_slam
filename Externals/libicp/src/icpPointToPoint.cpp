...
using namespace std;
#ifdef __KD_TREE_RELIABILITY
std::vector<int> __debug_v, __debug_v2;
#endif
namespace libicp {
// Also see (3d part): "Least-Squares Fitting of Two 3-D Point Sets" (Arun, Huang and Blostein)
    double IcpPointToPoint::fitStep(const double *T, const int32_t T_num, Matrix &R, Matrix &t,
        ...
        // init mean
        Matrix mu_m(1, dim);//mean of model data
        Matrix mu_t(1, dim);//mean of template data
...
//#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,r00,r01,r10,r11,t0,t1) reduction(+:mum0,mum1, mut0,mut1) // schedule (dynamic,2)
...
#ifdef __KD_TREE_RELIABILITY
                __debug_v2.push_back(result[0].idx);
#endif
                // set model point
            ...
            if(std::equal(__debug_v.begin(), __debug_v.end(),__debug_v2.begin(), [](int a, int b){return a==b;})){
              std::cerr<<"Vectors are not equal."<<std::endl;
              std::for_each(__debug_v.begin(), __debug_v.end(), [](int a){ std::cerr<<a<<",";});
              std::cerr<<std::endl;
              std::for_each(__debug_v2.begin(), __debug_v2.end(), [](int a){ std::cerr<<a<<",";});
              std::cerr<<std::endl;
              std::cerr<<std::endl;
            }
#endif
...
//#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,r00,r01,r02,r10,r11,r12,r20,r21,r22,t0,t1,t2) reduction(+:mum0,mum1,mum2, mut0,mut1,mut2) // schedule (dynamic,2)
    ...
    std::vector<int32_t> IcpPointToPoint::getInliers(const double *T, const int32_t T_num, const Matrix &R,
                                                     const Matrix &t, const double indist) {
        ...
        if (dim == 2) {
            ...
            for (int32_t i = 0; i < T_num; i++) {
                ...
                if (neighbor[0].dis < indist) {
                    inliers.push_back(i);
#ifdef __KD_TREE_RELIABILITY
                    __debug_v.push_back(i);
                  }else{
                    __debug_v.push_back(-1);
#endif
                }
